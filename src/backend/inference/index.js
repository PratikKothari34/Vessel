 ...
 *   generate(model, prompt, { numCtx })  -> string
 ...
 *
 * Chat and embedding are chosen SEPARATELY. One llama-server process serves one
 * model, so moving chat to llama.cpp does not move the embedder with it -- and
 * the embedder runs on CPU by default (EMBED_NUM_GPU=0), where Ollama costs
 * nothing to keep. INFERENCE_BACKEND moves chat + summarizer; EMBED_BACKEND
 * moves the embedder and defaults to staying on Ollama.
 */
...
module.exports = {
  ...
  generate: (model, prompt, opts) => chat.generate(model, prompt, opts),
